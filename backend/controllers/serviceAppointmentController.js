...
const stripe = stripeKey ? new Stripe(stripeKey, { apiVersion: "2026-11-15" }) : null;
...
// this will parse the time and return the am and pm
...
// this function will get the user from clerk and return user details
...
export const getServiceAppointments = async (req, res) => {
    try {
        ...
        // when the limit is freezed to 50  it will skip here
        const filter = {};
        ...
        return res.json({
            ...
            appointments,
            meta: {
                page, limit, total, count: application.length
            }
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        // check if all the fields are filled or not , if filled then update it
...
export const getServiceAppointmentStats = async (req, res) => {
    try {
        ...
        console.error(" getServiceAppointmentStat error:", error);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
